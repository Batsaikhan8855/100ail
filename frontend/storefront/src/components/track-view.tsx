...
export function TrackView() {
  ...
  // Нэг захиалгад олон нийлүүлэгч байвал хүргэлт нь тус тусдаа явна
  const [results, setResults] = useState<Tracking[] | null>(null);
  ...
  const track = async (value: string) => {
    ...
    try {
      setResults(
        await apiGet<Tracking[]>(
          `/deliveries/track/${encodeURIComponent(value.trim())}`,
        ),
      );
    } catch (cause) {
      setResults(null);
    ...
    <div className="min-h-screen bg-ink-950">
      ...
      <main className="mx-auto max-w-[820px] px-4 py-6 xl:px-6">
        <Panel>
          <PanelHeader
            title="Хүргэлт хянах"
            meta="Захиалгын дугаар эсвэл хянах код"
          />
          <form onSubmit={submit} className="flex flex-wrap gap-2 px-4 py-4">
            <label className="flex min-w-[220px] flex-1 items-center gap-2.5 rounded-md border border-ink-700 bg-ink-900 px-3 py-2.5">
              ...
              <span className="sr-only">Захиалгын дугаар эсвэл хянах код</span>
              <input
                value={code}
        ...
        {(results ?? []).map((result) => (
          <DeliveryPanel key={result.trackingCode} result={result} />
        ))}
      </main>
    </div>
  );
...
function DeliveryPanel({ result }: { result: Tracking }) {
  const activeStep = STEPS.findIndex((step) => step.id === result.status);
  ...
  return (
    <Panel className="mt-4">
      <PanelHeader
        ...
        action={<CopyButton value={result.trackingCode} label="Хянах код хуулах" />}
      />
      ...
      {result.position ? (
        <div className="border-t border-ink-700 px-4 py-4">
          ...
          <p className="mt-2.5 text-[11px] text-mute-dim">
            Байршил нь замд гарсан хугацаагаар тооцоологдсон ойролцоо утга.
            Жолоочийн GPS холбогдоход бодит цэг харагдана.
          </p>
        </div>
      ) : null}
