...
const TONE_CLASS: Record<Tone, string> = {
  neutral: "border-ink-600 bg-ink-800 text-mute",
  info: "border-info-border bg-info-bg text-info",
  warn: "border-brand-lo bg-brand/12 text-brand-hi",
  ok: "border-ok-border bg-ok-bg text-ok",
  bad: "border-danger-border bg-danger-bg text-danger",
};
...
export function StatCard({
  ...
  return (
    <div className="rounded-lg border border-ink-700 bg-ink-850 px-4 py-3.5">
      ...
      <div className="mt-1.5 text-[22px] font-semibold tracking-tight text-fg">
        {value}
      </div>
...
export function Button({
  ...
  const variants = {
    primary:
      "border-brand bg-brand text-on-brand hover:bg-brand-hi hover:border-brand-hi",
    ghost: "border-ink-600 bg-ink-900 text-mute hover:text-fg hover:border-mute-dim",
    danger: "border-danger-border bg-danger-bg text-danger hover:text-fg",
    quiet: "border-transparent bg-transparent text-mute hover:text-fg",
  };
  return (
    <button
      type="button"
...
const fieldClass =
  "w-full rounded-md border border-ink-600 bg-ink-900 px-3 text-[13px] text-fg placeholder:text-mute-dim outline-none transition-colors focus:border-brand";
...
export function Money({ value, className = "" }: { value: number; className?: string }) {
  return (
    <span className={`tabular-nums text-fg ${className}`}>
      {formatPrice(value)}
    </span>
  );
}
  ...
  return (
    <div className="flex h-40 items-end gap-1 px-4 py-4">
      {data.map((row) => (
        ...
        >
          <span className="pointer-events-none absolute -top-6 left-1/2 hidden -translate-x-1/2 whitespace-nowrap rounded border border-ink-600 bg-ink-900 px-1.5 py-0.5 text-[11px] text-fg group-hover:block">
